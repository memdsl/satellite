...
class Welcome(Container):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        app = cast(MainApp, self.app)
        app.add_logs("[b magenta]Start!")
...
class LocationLink(Static):
    ...
    def on_click(self) -> None:
        ...
        app.add_logs(f"Scrolling to [b]{self.reveal}[/b]")
...
class MainApp(App[None]):
    CSS_PATH = "main.tcss"
    TITLE = "Satellite"
    SUB_TITLE = "SoC Anglie Toolbox"
    BINDINGS = [
        ("q", "app.quit", "Quit"),
        ("m", "app.toggle_message", "Toggle Message"),
        ("d", "app.toggle_dark", "Toggle Dark Mode"),
        ("s", "app.take_screenshot", "Take Screenshot"),
        ("l", "app.toggle_class('RichLog', '-hidden')", "Show Logs"),
    ]

    show_sidebar = reactive(False)

    def add_logs(self, renderable: RenderableType) -> None:
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            ...
            Body(
                QuickAccess(
                    LocationLink("Configuration", ".location-top"),
                    LocationLink("FrontEnd-Integration", ".location-widgets"),
                    LocationLink("FrontEnd-Simulation", ".location-widgets"),
                    LocationLink("BlackEnd-Synthesis", ".location-rich"),
                    LocationLink("BlackEnd-Simulation", ".location-css")
                ),
    ...
    def on_mount(self) -> None:
        self.add_logs("Satellite is running")
    ...
    def action_take_screenshot(self, filename: str | None = None, path: str = "./") -> None:
        """Save an SVG "take_screenshot". This action will save an SVG file containing the current contents of the screen.

        Args:
            filename: Filename of take_screenshot, or None to auto-generate.
            path: Path to directory.
        """
        self.bell()
        path = self.save_screenshot(filename, path)
        message = f"Screenshot saved to [bold green]'{escape(str(path))}'[/]"
        self.add_logs(Text.from_markup(message))
        self.notify(message)
